function Vth=detectSpikeonsetVmP(vm, ratio_dvdt, vbound, dvdtlow, Fs, toplot)
...
for i=1:ntrs
    Vth.threshold{i}=[];
    v=vm(:, i);
    dv=my_smooth([0; diff(v)/(median(diff(t)))], 'moving', 3);
    std_dv=std(dv);
    ddv=my_smooth([0; diff(dv)/(median(diff(t)))], 'moving', 3);
    ...
    if any(dvpeaks)
        ...
        for j=1:length(dvpeaks)
            ...
            if peaklocs(j)-1*Fs/1000>0 && peaklocs(j)+1*Fs/1000<=length(dv)
                indpeaks=round([peaklocs(j)-1*Fs/1000:peaklocs(j)+1*Fs/1000]);
                ...
                if ~isempty(tfirst_on)
                    ...
                    if thcandidate-1*Fs/1000>0 && thcandidate+2*Fs/1000<=length(v) && max(v(thcandidate-1*Fs/1000: thcandidate+1*Fs/1000))>vbound(2) && max(v(thcandidate-1*Fs/1000: thcandidate+1*Fs/1000))-v(thcandidate)>vbound(4)
                        if toplot
                        ...
                        Vth.spks=[Vth.spks v(thcandidate-1*Fs/1000: thcandidate+2*Fs/1000)];
