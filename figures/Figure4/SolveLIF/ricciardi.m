function rate=ricciardi(mu,sigma,tau,VT,Vr)
...
if(VT<Vr)
    disp('Threshold lower than reset in function ricciardi! I am quitting :(');
end
if(sigma<0.)
    disp('Negative  noise variance in function ricciardi! I am quitting :(')
end
if(sigma==0)
    if(mu<VT)
        rate=0;
    else
        rate=1./(tau.*log((mu-Vr)./(mu-VT)));
    end
    return
end
xp=(mu-Vr)./sigma;
xm=(mu-VT)./sigma;
% if(xm>0)
%     rate=1./tau./(f_ricci(xp)-f_ricci(xm));
% elseif(xp>0)
%     rate=1./tau./(f_ricci(xp)+exp(xm.^2).*g_ricci(-xm));
% else
%     rate=exp(-xm.^2-log(g_ricci(-xm)-exp(xp.^2-xm.^2).*g_ricci(-xp)))./tau;
% end
f1=1./tau./(f_ricci(xp)-f_ricci(xm));
f2=1./tau./(f_ricci(xp)+exp(xm.^2).*g_ricci(-xm)) ;
...
function f_ricci=f_ricci(x)

z=x./(1+x);
f_ricci=log(2.*x+1)-2.2757881388024176*10^-1*z+7.7373949685442023*10^-1*z.^2-3.2056016125642045*10^-1*z.^3+3.2171431660633076*10^-1*z.^4-6.2718906618071668*10^-1*z.^5+9.3524391761244940*10^-1*z.^6-1.0616084849547165 *z.^7+6.4290613877355551*10^-1*z.^8-1.4805913578876898*10^-1*z.^9;

end
function g_ricci=g_ricci(x)
